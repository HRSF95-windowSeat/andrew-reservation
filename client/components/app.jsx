...
class App extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currTime: "6:00 PM",
      currDate: "2018-06-01",
      currTimeSlots: [{ party_size_max: 1 }],
      timeSlots: [{ party_size_max: 1 }],
      partySize: 2,
      time: "6:00 PM",
      date: "2018-06-01",
    }
  ...
  componentDidMount() {
    axios.get(`/reservations/restaurant/${this.state.restaurantId}/${this.state.currDate}`) //add /reservations for proxy
      .then((response) => {
        this.setState({
  ...
  findTable() {
    axios.get(`/reservations/restaurant/${this.state.restaurantId}/${this.state.currDate}`) //add /reservations for proxy
      .then((response) => {
        this.setState({
          timeSlots: response.data.time.split(','),
          isClicked: true,
  ...
  render() {
    return (
      <div className={`sticky-top col-lg-12 col-md-12 col-sm-12 col-xs-12 container  ${styles.reserveContainer} `}>
